 ...
 * 
 */
class Client1Socket extends SocketHelper {
...
public class Client1 extends Peer implements Runnable {
    ...
    public static void main(String[] args) {
        ...
        new Client1(serverListeningPort).Start();
    }

    public Client1(int serverPort) {                       // client constructors
        // TODO Auto-generated method stub
    ...
    public void Start() {
        ...
        try {
                ...
                public void run() {
                    try {
                        ...
                        while (!allPacketsReceived()) {
                            ...
                            for (int i = 0; i < Client1.packetIndex.size(); i++) {
                                int getIndex = Client1.packetIndex.get(i);
                                if (Client1.packetList.containsKey(getIndex)) {
                                    continue;
                                }

                                System.out.println("[" + Client1.peerName + "] ask peer-" + downloadPeer + " for packet " + getIndex);
                                ...
                                if (inDownStream.readInt() == 1) {                       //if he has that packet
                                    ...
                                    Client1.packetList.put(infoIndex, packet);
                                    savePackets(infoIndex, packet);
                                    Client1.mylist.add(infoIndex);

                                    System.out.println("[" + Client1.peerName + "] received packet " + packetIndex.get(i) + " from peer-" + downloadPeer);

                                } else {
                                    System.out.println("[" + Client1.peerName + "] peer-" + downloadPeer + " doesn't have packet " + getIndex);
                                }
                            }

                            System.out.println("[" + Client1.peerName + "] fetched available packets from peer-" + downloadPeer);

                            for (Integer storeIndex : Client1.packetIndex) {        // send for storage
                                if (!Client1.packetList.containsKey(storeIndex)) {
                                    continue;
                                }
                                postData(outUpStream, (Object) "store");
                                post(outUpStream, storeIndex);
                                postData(outUpStream, Client1.packetList.get(storeIndex));
                            }

                            System.out.println("[" + Client1.peerName + "] packets sent for storage to uploadpeer " + uploadPeer);
                            Thread.sleep(5000);
            ...
            while (true) {
                Client1Socket clientSocketObject = new Client1Socket();
                ...
                try {
                    ...
                    clientSocketObject.setPeerId(Client1.peerId);   // peerId for client1socket
    ...
    private void summary() {
        ...
        try {
            ...
            for (int i = 0; i < Client1.mylist.size(); i++) {
                stringBuilder.append(Client1.mylist.get(i) +1);
