 ...
 * 
 */
class Client5Socket extends SocketHelper {
...
public class Client5 extends Peer implements Runnable {
    ...
    public static void main(String[] args) {
        ...
        new Client5(serverListeningPort).Start();
    }

    public Client5(int serverPort) {                       // client constructors
        // TODO Auto-generated method stub
    ...
    public void Start() {
        ...
        try {
                ...
                public void run() {
                    try {
                        ...
                        while (!allPacketsReceived()) {
                            ...
                            for (int i = 0; i < Client5.packetIndex.size(); i++) {
                                int getIndex = Client5.packetIndex.get(i);
                                if (Client5.packetList.containsKey(getIndex)) {
                                    continue;
                                }

                                System.out.println("[" + Client5.peerName + "] ask peer-" + downloadPeer + " for packet " + getIndex);
                                ...
                                if (inDownStream.readInt() == 1) {                       //if he has that packet
                                    ...
                                    Client5.packetList.put(infoIndex, packet);
                                    savePackets(infoIndex, packet);
                                    Client5.mylist.add(infoIndex);

                                    System.out.println("[" + Client5.peerName + "] received packet " + packetIndex.get(i) + " from peer-" + downloadPeer);

                                } else {
                                    System.out.println("[" + Client5.peerName + "] peer-" + downloadPeer + " doesn't have packet " + getIndex);
                                }
                            }

                            System.out.println("[" + Client5.peerName + "] fetched available packets from peer-" + downloadPeer);

                            for (Integer storeIndex : Client5.packetIndex) {        // send for storage
                                if (!Client5.packetList.containsKey(storeIndex)) {
                                    continue;
                                }
                                postData(outUpStream, (Object) "store");
                                post(outUpStream, storeIndex);
                                postData(outUpStream, Client5.packetList.get(storeIndex));
                            }

                            System.out.println("[" + Client5.peerName + "] packets sent for storage to uploadpeer " + uploadPeer);
                            Thread.sleep(5000);
            ...
            while (true) {
                Client5Socket clientSocketObject = new Client5Socket();
                ...
                try {
                    ...
                    clientSocketObject.setPeerId(Client5.peerId);   // peerId for client1socket
    ...
    private void summary() {
        ...
        try {
            ...
            for (int i = 0; i < Client5.mylist.size(); i++) {
                stringBuilder.append(Client5.mylist.get(i) +1);
