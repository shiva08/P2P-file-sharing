 ...
 * 
 */
class Client3Socket extends SocketHelper {
...
public class Client3 extends Peer implements Runnable {
    ...
    public static void main(String[] args) {
        ...
        new Client3(serverListeningPort).Start();
    }

    public Client3(int serverPort) {                       // client constructors
        // TODO Auto-generated method stub
    ...
    public void Start() {
        ...
        try {
                ...
                public void run() {
                    try {
                        ...
                        while (!allPacketsReceived()) {
                            ...
                            for (int i = 0; i < Client3.packetIndex.size(); i++) {
                                int getIndex = Client3.packetIndex.get(i);
                                if (Client3.packetList.containsKey(getIndex)) {
                                    continue;
                                }

                                System.out.println("[" + Client3.peerName + "] ask peer-" + downloadPeer + " for packet " + getIndex);
                                ...
                                if (inDownStream.readInt() == 1) {                       //if he has that packet
                                    ...
                                    Client3.packetList.put(infoIndex, packet);
                                    savePackets(infoIndex, packet);
                                    Client3.mylist.add(infoIndex);

                                    System.out.println("[" + Client3.peerName + "] received packet " + packetIndex.get(i) + " from peer-" + downloadPeer);

                                } else {
                                    System.out.println("[" + Client3.peerName + "] peer-" + downloadPeer + " doesn't have packet " + getIndex);
                                }
                            }

                            System.out.println("[" + Client3.peerName + "] fetched available packets from peer-" + downloadPeer);

                            for (Integer storeIndex : Client3.packetIndex) {        // send for storage
                                if (!Client3.packetList.containsKey(storeIndex)) {
                                    continue;
                                }
                                postData(outUpStream, (Object) "store");
                                post(outUpStream, storeIndex);
                                postData(outUpStream, Client3.packetList.get(storeIndex));
                            }

                            System.out.println("[" + Client3.peerName + "] packets sent for storage to uploadpeer " + uploadPeer);
                            Thread.sleep(5000);
            ...
            while (true) {
                Client3Socket clientSocketObject = new Client3Socket();
                ...
                try {
                    ...
                    clientSocketObject.setPeerId(Client3.peerId);   // peerId for client1socket
    ...
    private void summary() {
        ...
        try {
            ...
            for (int i = 0; i < Client3.mylist.size(); i++) {
                stringBuilder.append(Client3.mylist.get(i) +1);
