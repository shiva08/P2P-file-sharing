 ...
 * 
 */
class Client4Socket extends SocketHelper {
...
public class Client4 extends Peer implements Runnable {
    ...
    public static void main(String[] args) {
        ...
        new Client4(serverListeningPort).Start();
    }

    public Client4(int serverPort) {                       // client constructors
        // TODO Auto-generated method stub
    ...
    public void Start() {
        ...
        try {
                ...
                public void run() {
                    try {
                        ...
                        while (!allPacketsReceived()) {
                            ...
                            for (int i = 0; i < Client4.packetIndex.size(); i++) {
                                int getIndex = Client4.packetIndex.get(i);
                                if (Client4.packetList.containsKey(getIndex)) {
                                    continue;
                                }

                                System.out.println("[" + Client4.peerName + "] ask peer-" + downloadPeer + " for packet " + getIndex);
                                ...
                                if (inDownStream.readInt() == 1) {                       //if he has that packet
                                    ...
                                    Client4.packetList.put(infoIndex, packet);
                                    savePackets(infoIndex, packet);
                                    Client4.mylist.add(infoIndex);

                                    System.out.println("[" + Client4.peerName + "] received packet " + packetIndex.get(i) + " from peer-" + downloadPeer);

                                } else {
                                    System.out.println("[" + Client4.peerName + "] peer-" + downloadPeer + " doesn't have packet " + getIndex);
                                }
                            }

                            System.out.println("[" + Client4.peerName + "] fetched available packets from peer-" + downloadPeer);

                            for (Integer storeIndex : Client4.packetIndex) {        // send for storage
                                if (!Client4.packetList.containsKey(storeIndex)) {
                                    continue;
                                }
                                postData(outUpStream, (Object) "store");
                                post(outUpStream, storeIndex);
                                postData(outUpStream, Client4.packetList.get(storeIndex));
                            }

                            System.out.println("[" + Client4.peerName + "] packets sent for storage to uploadpeer " + uploadPeer);
                            Thread.sleep(5000);
            ...
            while (true) {
                Client4Socket clientSocketObject = new Client4Socket();
                ...
                try {
                    ...
                    clientSocketObject.setPeerId(Client4.peerId);   // peerId for client1socket
    ...
    private void summary() {
        ...
        try {
            ...
            for (int i = 0; i < Client4.mylist.size(); i++) {
                stringBuilder.append(Client4.mylist.get(i) +1);
