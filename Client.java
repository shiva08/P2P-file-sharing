 ...
 * 
 */
class ClientSocket extends SocketHelper {
...
public class Client extends Peer implements Runnable {
    ...
    public static void main(String[] args) {
        ...
        new Client(serverListeningPort).Start();
    }

    public Client(int serverPort) {                       // client constructors
        // TODO Auto-generated method stub
    ...
    public void Start() {
        ...
        try {
                ...
                public void run() {
                    try {
                        ...
                        while (!allPacketsReceived()) {
                            ...
                            for (int i = 0; i < Client.packetIndex.size(); i++) {
                                int getIndex = Client.packetIndex.get(i);
                                if (Client.packetList.containsKey(getIndex)) {
                                    continue;
                                }

                                System.out.println("[" + Client.peerName + "] ask peer-" + downloadPeer + " for packet " + getIndex);
                                ...
                                if (inDownStream.readInt() == 1) {                       //if he has that packet
                                    ...
                                    Client.packetList.put(infoIndex, packet);
                                    savePackets(infoIndex, packet);
                                    Client.mylist.add(infoIndex);

                                    System.out.println("[" + Client.peerName + "] received packet " + packetIndex.get(i) + " from peer-" + downloadPeer);

                                } else {
                                    System.out.println("[" + Client.peerName + "] peer-" + downloadPeer + " doesn't have packet " + getIndex);
                                }
                            }

                            System.out.println("[" + Client.peerName + "] fetched available packets from peer-" + downloadPeer);

                            for (Integer storeIndex : Client.packetIndex) {        // send for storage
                                if (!Client.packetList.containsKey(storeIndex)) {
                                    continue;
                                }
                                postData(outUpStream, (Object) "store");
                                post(outUpStream, storeIndex);
                                postData(outUpStream, Client.packetList.get(storeIndex));
                            }

                            System.out.println("[" + Client.peerName + "] packets sent for storage to uploadpeer " + uploadPeer);
                            Thread.sleep(5000);
            ...
            while (true) {
                ClientSocket clientSocketObject = new ClientSocket();
                ...
                try {
                    ...
                    clientSocketObject.setPeerId(Client.peerId);   // peerId for client1socket
    ...
    private void summary() {
        ...
        try {
            ...
            for (int i = 0; i < Client.mylist.size(); i++) {
                stringBuilder.append(Client.mylist.get(i) +1);
